            ...
            for mslot in obj.material_slots:
                if self.alpha:
                    if not mslot.material.name.startswith("alpha_"):
                        ...
                        if (
                            "Image Texture" in ndt.nodes
                            and "Principled BSDF" in ndt.nodes
                        ):
                    ...
                    if mslot.material.name.startswith("alpha_"):
                        ...
                        if (
                            ...
                            for lnk in ndt.links:
                                if (
                                    lnk.from_socket == tex.outputs["Alpha"]
                                    and lnk.to_socket == bsdf.inputs["Alpha"]
                                ):
...
class CXMap_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = self.layout.row(align=True)
        row.operator(
            CXMap_SetAlpha.bl_idname, text="Enable", icon="DECORATE_ANIMATE"
        ).alpha = True
        row.operator(
            CXMap_SetAlpha.bl_idname, text="Disable", icon="DECORATE_KEYFRAME"
        ).alpha = False
